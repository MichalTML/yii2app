...
$this->registerJs('$(".groupe-create").click(function(event){   
                    ...
                    var groupName = $("#filegroupname-groupname").val(); 
                        if(groupName.length > 0){
                            ...
                            var url = "index.php?r=file-group/create&sygnature=" + sygnature + "&id=" + id;
                                $.ajax({
                                    url: url,
                                    type: "post",
                                    data: {action: action, groupName: groupName},
                                    success: function(msg) {
                                        $(".delete-group #filegroup-groupid").empty();
                                        $(".delete-group #filegroup-groupid").append("<option value=\\"\\"></option>");
                                        $(".add-group #filegroup-groupid").empty();
                                        $(".add-group #filegroup-groupid").append("<option value=\\"\\"></option>");
                                            $.each(msg.droptions, function(key, value) { 

                                                if(value.length > 0 ){
                                                    $(".delete-group #filegroup-groupid").append("<option value=" + key + ">" + value + "</option>");
                                                }
                                            });
                                            $.each(msg.droptions2, function(key, value) { 
                                                if(value.length > 0 ){
                                                        $(".add-group #filegroup-groupid").append("<option value=" + key + ">" + value + "</option>");
                                                }
                                            });
                                            }
                        });
                ...
                $(".groupe-delete").click(function(event){   
                    event.preventDefault();
                    var groupId = $(".delete-group #filegroup-groupid").val(); 
                        if(groupId.length > 0){
                ...
                $(".groupe-add").click(function(event){   
                    event.preventDefault();
                    var groupId = $(".add-group #filegroup-groupid").val(); 
                        if(groupId.length > 0){
